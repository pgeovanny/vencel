...
try{
  ...
  for(let turn=0;turn<Math.min(items.length,3);turn++){
    ...
    for(const o of objectives){const e=entity(stage,o);const ms=await worldClick(page,e,o.type,stage.visual?.camera_zoom||1);must(ms>=250&&ms<10000,'Clique gera caminhada perceptível',`${ms}ms`);await consume(page)}

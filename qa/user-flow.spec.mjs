...
const report={email,startedAt:new Date().toISOString(),checks:[],screenshots:[],timings:{}};
...
async function worldClick(page,e,kind){const vp=page.viewportSize(),p=point(e.position,vp.width,vp.height),start=Date.now();await page.mouse.click(p.x,kind==='actor'?p.y-34:p.y);await page.locator('.modal .sheet').waitFor({state:'visible',timeout:12000});return Date.now()-start}
async function consume(page){for(let i=0;i<8;i++){if(await page.locator('.dialog').count()){const b=page.locator('.dialog .choices button');if(await b.count()){await b.first().click();await page.waitForTimeout(160);continue}}const n=page.getByRole('button',{name:/Ir para o próximo objetivo|Voltar ao cenário/i});if(await n.count()){await n.first().click();return}if(!(await page.locator('.modal .sheet').count()))return;await page.waitForTimeout(120)}throw new Error('interaction did not finish')}
async function decisionModal(page){const b=page.locator('.guideBar button').filter({hasText:/Interagir/i});if(await b.count())await b.first().click();else await page.keyboard.press('e');await page.getByText('DECISÃO JURÍDICA',{exact:true}).waitFor({timeout:8000})}
...
try{
  ...
  for(let turn=0;turn<Math.min(items.length,3);turn++){
    ...
    const start=page.getByRole('button',{name:/Assumir ocorrência/i});if(await start.count())await start.click();
  ...
  const m=missions[0];await page.goto(`${BASE}/game/${m.id}`,{waitUntil:'networkidle'});const enter=page.getByRole('button',{name:/Entrar na ocorrência|Começar repetição/i});if(await enter.count())await enter.first().click();let tested=false;for(const st of [...(m.mission_json.stages||[])].sort((a,b)=>(a.order||0)-(b.order||0))){for(const o of st.objectives||[]){if(!['actor','object'].includes(o.type))continue;const e=entity(st,o);if(!e)continue;const ms=await worldClick(page,e,o.type);must(ms>=700,'Campanha usa a mesma caminhada do Plantão',`${ms}ms`);await consume(page);tested=true;break}if(tested)break}must(tested,'Campanha permite clique direto em NPC/evidência');await shot(page,'05-campaign');

  await page.goto(`${BASE}/archive`,{waitUntil:'networkidle'});must(/Arquivo|Casos/i.test(await text(page)),'Arquivo de Casos abre');await page.goto(`${BASE}/review`,{waitUntil:'networkidle'});must(/Revisão|memória/i.test(await text(page)),'Central de Revisão abre');const syllabus=(await rest('exam_syllabi?select=id,title&status=eq.published&limit=1',access))[0];if(syllabus){await page.goto(`${BASE}/syllabus/${syllabus.id}`,{waitUntil:'networkidle'});must(/DOMÍNIO DO EDITAL|Mapa/i.test(await text(page)),'Mapa do Edital abre')}

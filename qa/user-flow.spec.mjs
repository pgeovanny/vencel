...
async function worldClick(page,e,kind,zoom=1){await waitWorld(page);const target=page.locator('.dialog, .infoCard').first(),go=page.locator('.guideBar button').filter({hasText:/Ir até/i});if(await go.count()){const commandStart=Date.now();await go.first().waitFor({state:'visible',timeout:5000});await go.first().click({timeout:5000});const commandMs=Date.now()-commandStart;must(commandMs<COMMAND_MAX_EXPECTED_MS,'Comando Ir até responde sem atraso',`${commandMs}ms`);const walkStart=Date.now();await target.waitFor({state:'visible',timeout:30000});const ms=Date.now()-walkStart;report.timings.walks.push({kind,name:e.name||e.id,ms,commandMs,zoom,via:'guide-action'});return ms}const vp=page.viewportSize(),p=point(e.position,vp.width,vp.height,zoom),offsets=kind==='actor'?[[0,-28],[0,-8],[-24,-20],[24,-20]]:[[0,0],[-24,0],[24,0],[0,-24],[0,24]];for(const[dx,dy]of offsets){await page.mouse.click(p.x+dx,p.y+dy);const walkStart=Date.now();try{await target.waitFor({state:'visible',timeout:15000});const ms=Date.now()-walkStart;report.timings.walks.push({kind,name:e.name||e.id,ms,commandMs:0,zoom,via:'canvas'});return ms}catch{}}throw new Error(`interaction target did not open: ${e.name||e.id}`)}
...
try{
  ...
  for(let turn=0;turn<Math.min(items.length,3);turn++){
    ...
    for(const o of objectives){const e=entity(stage,o);const ms=await worldClick(page,e,o.type,stage.visual?.camera_zoom||1);log('Deslocamento automático conclui interação',true,`${ms}ms no runner gráfico`);await consume(page)}

...
async function worldClick(page,e,kind,zoom=1){await waitWorld(page);const start=Date.now(),target=page.locator('.dialog, .infoCard').first(),go=page.locator('.guideBar button').filter({hasText:/Ir até/i});if(await go.count()){await go.first().click();await target.waitFor({state:'visible',timeout:24000});const ms=Date.now()-start;report.timings.walks.push({kind,name:e.name||e.id,ms,zoom,via:'guide-action'});return ms}const vp=page.viewportSize(),p=point(e.position,vp.width,vp.height,zoom),offsets=kind==='actor'?[[0,-28],[0,-8],[-24,-20],[24,-20]]:[[0,0],[-24,0],[24,0],[0,-24],[0,24]];for(const[dx,dy]of offsets){await page.mouse.click(p.x+dx,p.y+dy);try{await target.waitFor({state:'visible',timeout:3200});const ms=Date.now()-start;report.timings.walks.push({kind,name:e.name||e.id,ms,zoom,via:'canvas'});return ms}catch{}}throw new Error(`interaction target did not open: ${e.name||e.id}`)}
...
try{
  ...
  for(let turn=0;turn<Math.min(items.length,3);turn++){
    ...
    for(const o of objectives){const e=entity(stage,o);const ms=await worldClick(page,e,o.type,stage.visual?.camera_zoom||1);must(ms>=350&&ms<24000,'Clique gera caminhada perceptível',`${ms}ms`);await consume(page)}
    await decisionModal(page);const choices=decision.choices||[];const idx=turn%Math.max(1,choices.length);await page.locator('.choices button').nth(idx).click();await page.locator('.modal .sheet').filter({hasText:/DECISÃO (CORRETA|INCORRETA)/}).waitFor({timeout:12000});must(/REGRA|APLICAÇÃO AO CASO|BASE LEGAL|MEMÓRIA DE PROVA/.test(await text(page)),'Feedback jurídico aparece estruturado');const next=page.getByRole('button',{name:/Próxima ocorrência|Ver relatório do Plantão/i});must(await next.count()>0,'Feedback libera continuação');await next.first().click();await page.waitForTimeout(900)
  }

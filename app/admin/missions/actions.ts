...
function lintMission(mj:any){
  ...
  for(const s of mj?.stages||[]){
    if(!s?.id) errors.push('Toda etapa precisa de id.');
    else if(stageIds.has(s.id)) errors.push(`Etapa duplicada: ${s.id}.`); else stageIds.add(s.id);
  ...
  for(const d of mj?.decisions||[]){
    if(!d?.id) errors.push('Toda decisão precisa de id.');
    else if(decisionIds.has(d.id)) errors.push(`Decisão duplicada: ${d.id}.`); else decisionIds.add(d.id);
    if(!Array.isArray(d?.choices)||d.choices.length<2) errors.push(`Decisão ${d?.id||'?'} precisa de pelo menos 2 alternativas.`);
    else if(d.choices.filter((c:any)=>c?.correct===true).length!==1) errors.push(`Decisão ${d.id} precisa ter exatamente 1 alternativa correta.`);
  }
...
export async function saveMissionAdmin(input:unknown){
  ...
  let mj:any;
  try{mj=JSON.parse(parsed.data.mission_json)}catch{return {ok:false,error:'O mission_json não é um JSON válido.'}}
  const issues=lintMission(mj);
  if(issues.length) return {ok:false,error:'A missão contém inconsistências.',issues};
  ...
  return error?{ok:false,error:error.message}:{ok:true,issues:[]};
}

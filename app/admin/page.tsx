...
export default async function Admin() {
  ...
  const [{count:profiles},{count:attempts},{count:missions},{count:completed},{count:presets},{count:activeGrants},{count:pendingReviews},{data:first},{data:settings},{data:catalog}]=await Promise.all([
  ...
  return <main className="adminShell">
    <section className="adminHero"><div><div className="ey">JURISQUEST CONTROL • COMMERCIAL BUILD</div><h1>Painel de Operações</h1><p>Conteúdo, usuários, direção visual, acesso e telemetria em uma única camada administrativa.</p></div><div className="adminHeroActions">{first&&<a className="btn primary" href={`/game/${first.mission_id}`}>Testar runtime</a>}<a className="btn" href="/dashboard">Área do aluno</a><a className="btn" href="/reset-password">Minha senha</a></div></section>
    ...
    <section className="adminModules">
      ...
      <div className="adminModule"><span className="moduleIcon">↻</span><div><small>RETENÇÃO</small><h2>Revisões</h2><p>{pendingReviews??0} revisões pendentes. A agenda agora é criada automaticamente no banco ao concluir a missão.</p><span className="adminBadge">1 • 7 • 30 DIAS</span></div></div>
      <div className="adminModule"><span className="moduleIcon">AI</span><div><small>GERAÇÃO DE CONTEÚDO</small><h2>Mission Agent</h2><p>Contrato V3 preparado para o agente que transformará edital em missões; o editor valida o JSON antes da publicação.</p><b>{settings?.agent_schema_version||'jurisquest.mission.v3-visual'}</b></div></div>
    </section>
